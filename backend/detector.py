...
PIPELINE_REVISION = "v15-semantic-arbitration"
...
def _ocr_label_is_compatible(cls_name: str, ocr_label: str) -> bool:
    semantic_type = _semantic_type_from_label(ocr_label)
    if semantic_type is not None:
        return semantic_type == cls_name
...
def _resolve_semantic_proposal_conflicts(proposals: list[dict]) -> tuple[list[dict], list[dict]]:
    """Keep the most specific OCR interpretation when two types share one visual anchor."""
    active: list[dict] = []
    alternatives: list[dict] = []
    ranked = sorted(
        proposals,
        key=lambda item: (
            int((item.get("ocrEvidence") or {}).get("semanticStackDepth", 1)),
            len(re.findall(r"[a-z0-9]+", str(item.get("ocrLabel") or "").lower())),
    ...
    for proposal in ranked:
        conflict = next(
            (
                ...
                if selected.get("type") != proposal.get("type")
                and _bbox_iou(selected["bbox"], proposal["bbox"]) >= 0.80
            ),
            None,
        )
        if conflict is None:
...
def _resolve_semantic_conflicts(
    ...
    """Abstain from a YOLO hypothesis when nearby explicit OCR supports another known type."""
    width, height = image_size
    active = []
    alternatives = []
    for component in supervised:
        evidence = component.get("ocrEvidence") or {}
        semantic_type = evidence.get("semanticType")
        replacement = None
        replacement_reason = None
        if evidence.get("accepted") is False and semantic_type:
            evidence_bbox = evidence.get("bbox") or component.get("bbox")
            if evidence_bbox:
                evidence_center = (
                ...
                for proposal in proposals:
                    if proposal.get("type") != semantic_type:
                        continue
                    proposal_evidence = proposal.get("ocrEvidence") or {}
                    proposal_bbox = proposal_evidence.get("bbox") or proposal.get("bbox")
                    if not proposal_bbox:
                        continue
                    proposal_center = (
                        (proposal_bbox[0] + proposal_bbox[2]) / 2,
                        (proposal_bbox[1] + proposal_bbox[3]) / 2,
                    )
                    if (
                        abs(proposal_center[0] - evidence_center[0]) <= width * 0.08
                        and abs(proposal_center[1] - evidence_center[1]) <= height * 0.06
                    ):
                        replacement = proposal
                        replacement_reason = "explicit_semantic_ocr_conflict"
                        break
        if replacement is None:
            ...
            if component_bbox:
                ...
                for proposal in proposals:
                    ...
                    normalized_x = abs(proposal_center[0] - component_center[0]) / max(width, 1)
                    normalized_y = abs(proposal_center[1] - component_center[1]) / max(height, 1)
                    if normalized_x <= 0.090 and normalized_y <= 0.090:
                        nearby.append((normalized_x + normalized_y, proposal))
                semantic_agreement = any(
                    proposal.get("type") == component.get("type") for _, proposal in nearby
                )
                conflicts = [
                    item for item in nearby if item[1].get("type") != component.get("type")
                ]
                if conflicts and not semantic_agreement:
                    _, replacement = min(conflicts, key=lambda item: item[0])
                    replacement_reason = "semantic_anchor_spatial_conflict"
        if replacement is None:
            ...
            **component,
            ...
            "metadata": {
                **(component.get("metadata") or {}),
                ...
                "supersededReason": replacement_reason,
            },
        })
    return active, alternatives
...
def detect(image_path: str) -> dict | None:
    ...
    for box in detected_boxes:
        ...
        if ocr_evidence:
            ...
            semantic_type = _semantic_type_from_label(str(ocr_evidence.get("text") or ""))
            ocr_evidence = {
                **ocr_evidence,
                "accepted": compatible,
    ...
    ocr_proposals, proposal_alternatives = _resolve_semantic_proposal_conflicts(ocr_proposals)
    components, detection_alternatives = _resolve_semantic_conflicts(
        components,
        ocr_proposals,
        (img_w, img_h),
    )
    detection_alternatives.extend(proposal_alternatives)
    ...
    review_items.extend(
        {
            ...
            "id": component["id"],
            "reason": (
                f"Supervised class '{component['type']}' conflicts with explicit OCR semantics and "
                f"was superseded by '{(component.get('metadata') or {}).get('supersededBy')}'."
            ),
        }
        for component in detection_alternatives
    )
    review_items.extend(
        {
            "kind": "ocr_label",
